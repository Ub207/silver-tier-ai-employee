...
def process_email(filepath: Path, meta: dict, dry: bool) -> dict:
    ...
    if not dry:
        ...
        print(f"    [AI] Drafting reply options for email...")
        ai = _classify_and_draft(sender, original)
        ...
        # Write reply-draft approval file to Pending_Approval/
        financial_warning = ""
        if ai.get("financial_flag"):
            financial_warning = """\n> [!WARNING]
> **FINANCIAL FLAG**: Invoice/payment content detected.
> Confirm amount is < PKR 10,000 before approving.
> If >= PKR 10,000, escalate for secondary approval.\n"""

        option_d_block = ""
        if ai.get("option_d"):
            option_d_block = f"\n**Option D — Escalation / Info Request (FINANCIAL_FLAG):**\n> {ai['option_d']}\n"

        approval_path = PENDING / f"EMAIL_REPLY_{filepath.name}"
        approval_path.write_text(f"""---
type: email_reply
original_file: {filepath.name}
from: {sender}
...
> **AI Recommendation:** {ai.get("recommendation", "Option A for professional emails")}
...
## To Send
Move this file to `Approved/` — approval_executor.py will send via SMTP.
""", encoding="utf-8")
...
def update_dashboard(results: list[dict]):
    ...
    # ── WhatsApp items from current run ──────────────────────────────────────
    for r in results:
    ...
    # ── Pending Approvals — scan actual folder ────────────────────────────────
    approval_rows = []
    pending_files = sorted(PENDING.glob("*.md"), key=lambda f: f.stat().st_mtime, reverse=True)
    for pf in pending_files:
        try:
            meta = parse_frontmatter(pf.read_text(encoding="utf-8", errors="replace"))
            item_type = meta.get("type", "unknown")
            if item_type == "whatsapp_reply":
                action = "Select reply option (A/B/C) and move to Approved/"
            elif item_type == "email_reply":
                action = "Review reply draft and move to Approved/"
            elif item_type in ("linkedin_post", "linkedin_company"):
                action = "Review post → open browser → click Post"
            else:
                action = "Review and move to Approved/ or Rejected/"
            created = meta.get("created", "—")
            approval_rows.append(f"| {pf.name} | {item_type} | {action} |")
        except Exception:
            approval_rows.append(f"| {pf.name} | — | Review required |")

    # ── Recent Plans — scan Plans folder (latest 10) ──────────────────────────
    plan_rows = []
    plan_files = sorted(PLANS.glob("PLAN_*.md"), key=lambda f: f.stat().st_mtime, reverse=True)
    for pf in plan_files[:10]:
        try:
            meta = parse_frontmatter(pf.read_text(encoding="utf-8", errors="replace"))
            context = meta.get("context", pf.stem[:40])
            created = meta.get("created", "—")
            plan_rows.append(f"| {pf.name} | {context[:50]} | {created} |")
        except Exception:
            plan_rows.append(f"| {pf.name} | — | — |")
