...
CEO_BRIEFING_DOW   = 6      # day of week to generate CEO briefing (0=Mon, 6=Sun)
...
def run_ceo_briefing():
    """Generate Monday Morning CEO Briefing (runs Sundays)."""
    log_path = LOG_DIR / "ceo_briefing.log"
    logger.info("Running ceo_briefing.py ...")
    with open(log_path, "a", encoding="utf-8") as lf:
        lf.write(f"\n\n--- Run at {datetime.now().isoformat()} ---\n")
        result = subprocess.run(
    ...
    if result.returncode != 0:
        logger.warning("ceo_briefing.py exited with code %d", result.returncode)
    else:
...
def main():
    ...
    last_briefing_date = None  # track last CEO briefing day
    ...
    try:
        while True:
            ...
            # CEO Briefing: runs once on Sunday (day 6)
            today = datetime.now()
            if today.weekday() == CEO_BRIEFING_DOW and last_briefing_date != today.date():
                logger.info("Sunday detected — generating CEO Briefing ...")
                run_ceo_briefing()
                last_briefing_date = today.date()
